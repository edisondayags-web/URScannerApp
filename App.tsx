...
// import JailMonkey from 'jail-monkey'; // DISABLED
...
// if (JailMonkey.isJailBroken()) { // DISABLED  
...
if (DeviceInfo.isEmulatorSync()) {  
  ...
  // Sentry.captureMessage('Emulator detected', 'warning'); // DISABLED  
}  
...
const field = 'selected_' + wallet.name.replace(/\s/g, '');
try {
await db.collection('live_stats').doc('summary').update({ total_app_select_clicks: firestore.FieldValue.increment(1), [field]: firestore.FieldValue.increment(1) });
} catch (err) {
const raw = storage.getString('pending_clicks');
...
await db.collection('click_logs').add({ action: 'clicked_open_in' + wallet.name, merchant: extractMerchantName(currentQR || ''), qr_hash: currentQR? currentQR.slice(-6) : '', timestamp: firestore.FieldValue.serverTimestamp(), platform: Platform.OS, disclaimer: 'User clicked button only. No payment confirmed.' });
